export default function Index ({ html, state }) {
  const { store } = state
  const { accountID, dataID, conversation } = store

  return html`
    <ui-message-list></ui-message-list>
    <p class="">
    ...
    <script type="module">
      import { init, connect, send } from '/_public/ws.js'
...
//     let result = await data.lore.get({ accountID, dataID })
//     if (!result) return {
//       statusCode: 302,
//       headers: { location: '/' },
//     }
